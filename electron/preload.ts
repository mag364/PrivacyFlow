...
contextBridge.exposeInMainWorld('privacyflow', {
  ...
  mail: {
    openDraft: (input: { to: string; cc?: string; subject: string; body: string; bodyHtml?: string }) => ipcRenderer.invoke('mail:openDraft', input),
  },
  external: {
  ...
  graph: {
    ...
    sendMail: (input: { accessToken: string; to: string; cc?: string; subject: string; body: string; bodyHtml?: string; saveToSentItems?: boolean }) =>
      ipcRenderer.invoke('graph:sendMail', input),
  },
  outlook: {
    accounts: () => ipcRenderer.invoke('outlook:accounts'),
    openDraft: (input: { accountEmail?: string; to: string; cc?: string; subject: string; body: string; bodyHtml?: string }) =>
