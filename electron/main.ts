...
function writeUpdaterScript(input: { zipPath: string; appFolder: string; lockPath: string }): { scriptPath: string; cmdPath: string; logPath: string } {
  ...
  const lines = [
    ...
    '  $AppAsarPath = Join-Path $AppFolder "resources\\app.asar"',
    '  Write-Log "Checking whether the application files are in use."',
    '  $AppFilesUnlocked = $false',
    '  for ($i = 0; $i -lt 600; $i++) {',
    '    if (Test-FileUnlocked $AppAsarPath) {',
    '      $AppFilesUnlocked = $true',
    '      break',
    '    }',
    '    if ($i -eq 6) {',
    '      Write-Log "PrivacyFlow is still open on this or another PC. Waiting up to five minutes for all users to close it."',
    '      try {',
    '        Add-Type -AssemblyName System.Windows.Forms',
    '        [System.Windows.Forms.MessageBox]::Show(',
    '          "PrivacyFlow is still open on this or another PC. Close PrivacyFlow on every PC using the shared application folder, then select OK. The updater will continue automatically.",',
    '          "PrivacyFlow update waiting",',
    ...
    '    Start-Sleep -Milliseconds 500',
    '  }',
    '  if (-not $AppFilesUnlocked) {',
    '    throw "PrivacyFlow is still open on this or another PC after five minutes. Close every PrivacyFlow window and run the update again."',
    '  }',
    '  Write-Log "Application files are available for update."',
    ...
    '  if (-not (Test-Path (Join-Path $BackupFolder "PrivacyFlow.exe")) -or -not (Test-Path (Join-Path $BackupFolder "resources\\app.asar"))) {',
    '    throw "The application backup could not be validated; no files were replaced."',
    '  }',
    ...
    '  if (-not (Test-Path (Join-Path $AppFolder "PrivacyFlow.exe")) -or -not (Test-Path (Join-Path $AppFolder "resources\\app.asar"))) {',
    '    throw "The updated application files could not be validated."',
    '  }',
    '  $ReplacementStarted = $false',
    ...
    '  if ($ReplacementStarted -and $BackupFolder -and (Test-Path -LiteralPath $BackupFolder)) {',
    '    Write-Log "Replacement failed after it started; restoring the previous application backup."',
    '    try {',
    '      Get-ChildItem -LiteralPath $AppFolder -Force -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction Stop',
    '      Get-ChildItem -LiteralPath $BackupFolder -Force | Copy-Item -Destination $AppFolder -Recurse -Force -ErrorAction Stop',
    '      if (-not (Test-Path (Join-Path $AppFolder "PrivacyFlow.exe"))) { throw "Restored PrivacyFlow.exe was not found." }',
    '      $RollbackSucceeded = $true',
    '      Write-Log "Previous application version restored successfully."',
    '    } catch {',
    ...
    '    Write-Log "Restarting the restored PrivacyFlow version."',
    '    Start-Process -FilePath (Join-Path $AppFolder "PrivacyFlow.exe") -WorkingDirectory $AppFolder',
    '  }',
